...
# USER SETTINGS
# =========================================================
HYSYS_CASE_PATH = r"D:\BUUU\working file.hsc"

ABSORBER_NAME = "Absorber"                    # Unit op name in HYSYS
...
CLEAN_GAS_STREAM_NAME = "Clean gas"           # Clean gas outlet stream
...
FAN_K100_NAME = "K-100"                       # K-100 Fan
FAN_STREAM_1_NAME = "1"                       # K-100 inlet stream (feed)
FAN_STREAM_2_NAME = "2"                       # K-100 outlet stream (product)

# Column stages / packing height range to optimize
STAGE_VALUES = list(range(18, 26, 2))  
...
# Stage-dependent pressure settings for K-100 Fan and Absorber COL1
# Stream 1 pressure is always 101 kPa
FAN_STREAM_1_PRESSURE_KPA = 101.0
# Mapping: stages -> (Stream 2 pressure kPa, ΔP kPa, Absorber COL1 Pn kPa)
STAGE_PRESSURE_MAP = {
    24: {"stream2_kPa": 113.0, "delP_kPa": 12.0, "col1_Pn_kPa": 113.0},
    22: {"stream2_kPa": 112.0, "delP_kPa": 11.0, "col1_Pn_kPa": 112.0},
    20: {"stream2_kPa": 111.0, "delP_kPa": 10.0, "col1_Pn_kPa": 111.0},
    18: {"stream2_kPa": 110.0, "delP_kPa":  9.0, "col1_Pn_kPa": 110.0},
}

# Base case for Absorber CAPEX scaling
BASE_PACKING_HEIGHT = 18
BASE_COST_SS_2022_EUR = 1127200.0             # Base purchased cost (SS) in 2022 EUR
EXPONENT = 0.9
...
A1_M2 = 2747.0                 # Reference area (m²) for base cost C1_SS_2021_EUR
...
# K-100 Fan cost parameters
FAN_BASE_COST_EUR = 90600.0                   # Base purchased cost (EUR)
FAN_BASE_CAPACITY_M3PH = 190800.0             # Base capacity (m3/hr)
FAN_COST_EXPONENT = 0.6                       # Scaling exponent
FAN_INSTALL_FACTOR = 5.3                      # Installation factor (same as generic)
...
# HELPER FUNCTIONS
# =========================================================
def _call_with_timeout(func, timeout=6, default=None):
    """
    Call func() in a daemon thread with a hard timeout.
    Returns (result, True) on success, (default, False) if the call timed out.
    Raises the exception if func() raised one (within the timeout window).
    """
    result = [default]
    exc    = [None]
    ...
    t.join(timeout)
    if t.is_alive():
        # COM call did not return within timeout — treat as "not done yet"
...
def wait_for_convergence(case, max_wait_s=300, poll_s=2.0):
    ...
    Each COM poll is wrapped with a hard 6-second timeout so a frozen
    HYSYS call can never block the script indefinitely.
    """
    start_time = time.time()
    print("  Waiting for HYSYS solver...", end="", flush=True)

    # Cache the Solver object once (avoids repeated COM dispatch)
    solver = None
    try:
        val, ok = _call_with_timeout(lambda: case.Solver, timeout=6)
        if ok:
            solver = val
    except Exception:
        pass
    ...
    while time.time() - start_time < max_wait_s:
        ...
        try:
            if solver is not None:
                val, ok = _call_with_timeout(lambda: solver.IsSolving, timeout=6)
            else:
                val, ok = _call_with_timeout(lambda: case.Solver.IsSolving, timeout=6)
            # ok=False means COM call timed out — assume still solving
            if ok:
                is_solving = bool(val)
        except Exception:
            is_solving = True  # COM error → be safe, keep waiting
...
def wait_for_column_convergence(absorber, case, max_wait_s=180, poll_s=2.0):
    ...
    Every COM call is wrapped with a hard 6-second timeout so the script
    can never hang on a frozen HYSYS COM call.
    """
    start = time.time()
    print("  Waiting for absorber column to converge...", end="", flush=True)

    while time.time() - start < max_wait_s:
        # ── Primary: ColumnFlowsheet.Converged (green bar = True in HYSYS) ──
        try:
            val, ok = _call_with_timeout(
                lambda: absorber.ColumnFlowsheet.Converged, timeout=6
            )
            if ok and val:
                print(" Column converged.")
                return True
            # ok=False means COM call timed-out → HYSYS still busy, keep waiting
        except Exception:
            pass

        # ── Secondary: main solver idle → assume column done ──
        try:
            val, ok = _call_with_timeout(
                lambda: case.Solver.IsSolving, timeout=6
            )
            if ok and not bool(val):          # solver has stopped
                time.sleep(poll_s)
                try:
                    val2, ok2 = _call_with_timeout(
                        lambda: absorber.ColumnFlowsheet.Converged, timeout=6
                    )
                    if ok2 and val2:
...
def get_component_massflow_kgph(stream, comp_name="CO2"):
    ...
    try:
        ...
        if comp_idx == -1:
            raise RuntimeError(
                f"Component '{comp_name}' not found in fluid package for stream '{getattr(stream, 'name', '<unknown>')}'."
            )
    ...
    raise RuntimeError(
        f"Could not read component mass flow for '{comp_name}' from stream '{getattr(stream, 'name', '<unknown>')}'."
    )
...
def get_co2_mole_fraction(stream, comp_name="CO2"):
    ...
    raise RuntimeError(
        f"Could not read CO2 mole fraction from stream '{getattr(stream, 'name', '<unknown>')}'."
    )
...
def _set_stream_pressure(stream, pressure_kpa):
    """
    Set the pressure of a HYSYS material stream in kPa.
    """
    prop = stream.Pressure
    if hasattr(prop, "SetValue"):
        prop.SetValue(float(pressure_kpa), "kPa")
        return
    if hasattr(prop, "Value"):
        # Value is in internal units (kPa for HYSYS)
        prop.Value = float(pressure_kpa)
        return
    raise RuntimeError(
        f"Could not set pressure on stream '{getattr(stream, 'Name', stream)}'."
    )


def _set_absorber_col1_pressure(absorber, pressure_kpa):
    """
    Set the Pn (stage pressure) of the absorber column's COL1
    packing/tray section to the given value in kPa.
    Navigates ColumnFlowsheet -> Operations to find 'COL1', 'MAIN TS', or 'MAIN TOWER'.
    """
    errors = []
    ...
    try:
        ...
        for i in range(ops.Count):
            try:
                ...
                sub_name = ""
                try:
                    sub_name = sub_op.name # Try lowercase 'name' for HYSYS 14
                except Exception:
                    try:
                ...
                if sub_name.upper() in ["COL1", "MAIN TS", "MAIN TOWER"]:
                    # Try setting Pn / PressureSpec / Pressure / BottomStagePressure
                    for pname in ("Pn", "PressureSpec", "Pressure", "BottomStagePressure",
                                  "StagePressure", "BottomPressure", "TopStagePressure"):
                        if hasattr(sub_op, pname):
                            prop = getattr(sub_op, pname)
                            try:
                                if hasattr(prop, "SetValue"):
                                    prop.SetValue(float(pressure_kpa), "kPa")
                                    return
                                ...
                                errors.append(f"{sub_name}.{pname}: {e}")
            except Exception as e:
                errors.append(f"Operations.Item({i}): {e}")
    except Exception as e:
        errors.append(f"ColumnFlowsheet.Operations: {e}")

    # Path 2: ColumnFlowsheet.ColumnStages -> set stage pressure
    try:
        stages = absorber.ColumnFlowsheet.ColumnStages
        # Set pressure on last stage (Pn = bottom stage pressure)
        last_stage = stages.Item(stages.Count - 1)
        for pname in ("Pressure", "StagePressure", "P"):
            if hasattr(last_stage, pname):
                prop = getattr(last_stage, pname)
                try:
                    if hasattr(prop, "SetValue"):
                        prop.SetValue(float(pressure_kpa), "kPa")
                        return
                    ...
                    errors.append(f"LastStage.{pname}: {e}")
    except Exception as e:
        errors.append(f"ColumnStages path: {e}")

    raise RuntimeError(
        f"Could not set Absorber COL1 Pn to {pressure_kpa} kPa. "
        f"Details: {errors}"
    )
...
def calc_fan_capex_eur(new_capacity_m3ph):
    """
    K-100 Fan CAPEX:
    C_new = C_base * (new_capacity / base_capacity) ^ 0.6
    Then apply installation factor.
    """
    if new_capacity_m3ph <= 0:
        return float("nan")
    c_new = FAN_BASE_COST_EUR * (
        (new_capacity_m3ph / FAN_BASE_CAPACITY_M3PH) ** FAN_COST_EXPONENT
    )
    return c_new * FAN_INSTALL_FACTOR


def calc_fan_annual_opex_eur_per_year(duty_kjph):
    """
    K-100 Fan annual OPEX:
    Annual OPEX = (duty_kJ/h / 3600 -> kW) * operating_hours * electricity_price
    """
    duty_kw = abs(duty_kjph) / 3600.0
    return duty_kw * OPERATING_HOURS_PER_YEAR * ELECTRICITY_PRICE_EUR_PER_KWH


def get_fan_capacity_m3ph(fan):
    """
    Read K-100 fan actual feed volume flow (capacity) in m3/h.
    Tries the fan's own properties first, then falls back to feed stream.
    """
    candidates = [
        "Capacity", "capacity",
        "ActualVolumeFlow", "VolumeFlow", "VolumetricFlow",
        "FeedActualVolumeFlow",
    ]
    units = ["m3/h", "m3/hr", "m^3/h", "m^3/hr"]
    errors = []
    for name in candidates:
        if hasattr(fan, name):
            prop = getattr(fan, name)
            try:
                return _try_get_value(prop, units)
            except Exception as e:
                errors.append(f"{name}: {e}")

    # Fallback: read from feed (inlet) stream
    feed_stream = None
    for feed_attr in ("FeedStream", "Feed", "InletStream", "Inlet"):
        if hasattr(fan, feed_attr):
            try:
                feed_stream = getattr(fan, feed_attr)
                break
            except Exception:
                pass
    ...
    raise RuntimeError(
        f"Could not read fan capacity (m3/h) from '{getattr(fan, 'Name', fan)}'. "
        f"Tried properties {candidates} and feed stream volumetric flow. "
        f"Details: {errors}"
    )


def get_fan_duty_kjph(fan):
    """
    Read K-100 fan duty in kJ/h.
    Tries direct property, then energy stream.
    """
    candidates = ["Duty", "HeatFlow", "Power", "Energy", "Q"]
    for c in candidates:
        if hasattr(fan, c):
            v = getattr(fan, c)
            try:
    ...
    # Fallback: energy stream
    for eattr in ("EnergyFeed", "EnergyStream", "EnergyConnection",
                  "PowerFeed", "WorkStream"):
        if hasattr(fan, eattr):
            try:
                energy_stream = getattr(fan, eattr)
                for eprop in ("HeatFlow", "Power", "Duty", "EnergyFlow", "Q"):
                    if hasattr(energy_stream, eprop):
                        prop = getattr(energy_stream, eprop)
                        try:
                            if hasattr(prop, "GetValue"):
                                return float(prop.GetValue("kJ/h"))
                            if hasattr(prop, "Value"):
                                return float(prop.Value)
                            return float(prop)
                        except Exception:
    ...
    raise RuntimeError(
        f"Could not read duty (kJ/h) from fan '{getattr(fan, 'Name', fan)}'."
    )
...
def set_co2_stage_efficiency(absorber, co2_eff=0.15, comp_name="CO2"):
    ...
    When a new stage is added (e.g. 18→19), HYSYS sets its CO2 eff = 1.0
    by default. This function resets it to 0.15 before Run is called.
    Returns: (stages_set, total_stages)
    """
    errors = []
    ...
    try:
        ...
        # Find column section sub-op — use any sub-op with a stage-count property
        col1 = None
        col1_name = ""
        ...
        for i in range(int(comps.Count)):
            ...
            if str(cname).strip().upper() == comp_name.upper():
                ...
                break
        if co2_idx == -1:
            raise RuntimeError(f"Component '{comp_name}' not found in fluid package.")
        print(f"  [eff] CO2 is component index {co2_idx} in COL1.")

        # Get stage count
        n_stages = 0
        for prop in STAGE_COUNT_PROPS:
            if hasattr(col1, prop):
                try:
                    val = getattr(col1, prop)
                    n_stages = int(float(val.Value) if hasattr(val, "Value") else float(val))
                    break
                except Exception:
                    pass
        if n_stages == 0:
        ...
        for stages_attr in ("Stages", "TrayStages", "ColumnStages", "Trays", "Tray"):
            if hasattr(col1, stages_attr):
                try:
                    ...
                    if set_count > prev:
                        print(f"  [eff] Path 2 ({stages_attr}): set {set_count-prev}/{n_stages} stages.")
                        if set_count >= n_stages:
                            return set_count, n_stages
                except Exception as e:
                    errors.append(f"Path2 {stages_attr}: {e}")
            else:
                # Debug: if it doesn't have it, we might want to know
                # But only if we are desperate.
                pass

        # Path 3: SetComponentEfficiency method
            ...
            try:
                ...
                for s in range(n_stages):
                    try:
                        ...
                        errors.append(f"Path3 s={s}: {e}")
                if set_count > prev:
                    print(f"  [eff] Path 3 (SetComponentEfficiency): set {set_count-prev}/{n_stages} stages.")
            except Exception as e:
                errors.append(f"Path3: {e}")

        if set_count == 0:
            raise RuntimeError(f"Could not set CO2 efficiency on any stage. Details: {errors}")
        if set_count < n_stages:
...
def plot_results(results):
    ...
    # Show all plots — block=False so the script continues and exits cleanly.
    # Close the plot windows manually when you are done viewing them.
    print("  (Close the plot windows to finish the script.)")
    plt.show(block=True)
...
def main():
    ...
    try:
        ...
        # K-100 fan inlet / outlet material streams
        fan_stream_1 = streams.Item(FAN_STREAM_1_NAME)
        fan_stream_2 = streams.Item(FAN_STREAM_2_NAME)
    ...
    for n_stages in STAGE_VALUES:
        ...
        # ── Base case: already converged, skip stage-change & Run ─────────
        ...
        if is_base_case:
            ...
            # Step 2: Set stage-dependent pressures (K-100 streams & Absorber COL1)
            if n_stages in STAGE_PRESSURE_MAP:
                pmap = STAGE_PRESSURE_MAP[n_stages]
                s2_kpa = pmap["stream2_kPa"]
                col1_pn_kpa = pmap["col1_Pn_kPa"]
                try:
                    case.Solver.CanSolve = False
                    time.sleep(0.3)
                    # _set_absorber_col1_stages(absorber, n_stages) # redundant call
                    # print(f"  Set Absorber COL1 number of stages = {n_stages}")
                    _set_stream_pressure(fan_stream_1, FAN_STREAM_1_PRESSURE_KPA)
                    print(f"  Set Stream '{FAN_STREAM_1_NAME}' pressure = {FAN_STREAM_1_PRESSURE_KPA} kPa")
                    _set_stream_pressure(fan_stream_2, s2_kpa)
                    print(f"  Set Stream '{FAN_STREAM_2_NAME}' pressure = {s2_kpa} kPa")
                    _set_absorber_col1_pressure(absorber, col1_pn_kpa)
                    print(f"  Set Absorber COL1 Pn = {col1_pn_kpa} kPa")
                    case.Solver.CanSolve = True
                except Exception as e:
                    print(f"  WARNING: Could not set pressures: {e}")
                    try:
                        case.Solver.CanSolve = True
                    except Exception:
                        pass
            else:
                print(f"  WARNING: No pressure mapping for {n_stages} stages; pressures unchanged.")

            # Step 3: Set CO2 efficiency = 0.15 on ALL stages (incl. new)
            try:
                n_set, n_total = set_co2_stage_efficiency(
                    absorber, co2_eff=CO2_STAGE_EFFICIENCY, comp_name="CO2"
                )
                print(f"  CO2 efficiency set to {CO2_STAGE_EFFICIENCY} on {n_set}/{n_total} stages.")
            except Exception as e:
                print(f"  WARNING: Could not set CO2 stage efficiency: {e}")
                print(f"  Continuing — efficiency may be wrong for new stage(s).")

            # Step 4: Press Run and wait
        ...
        try:
            ...
            # K-100 Fan
            k100_capacity_m3ph = get_fan_capacity_m3ph(k100)
            k100_duty_kjph = get_fan_duty_kjph(k100)
        ...
        # K-100 Fan
        k100_capex = calc_fan_capex_eur(k100_capacity_m3ph)
        k100_annual_opex = calc_fan_annual_opex_eur_per_year(k100_duty_kjph)
        ...
        row = {
            ...
            # K-100 Fan
            "K100_Capacity_m3_per_h": k100_capacity_m3ph,
            "K100_Duty_kJ_per_h": k100_duty_kjph,
            "K100_CAPEX_EUR": k100_capex,
            "K100_AnnualOPEX_EUR_per_y": k100_annual_opex,
        ...
        print(f"    K-100 Fan                   : {k100_capex:>15,.0f}  EUR")
        print(f"      Capacity (act feed vol)    : {k100_capacity_m3ph:>12,.1f}  m3/h")
        print(f"    {'─'*52}")
        ...
        print(f"    K-100 Fan electricity       : {k100_annual_opex:>15,.0f}  EUR/yr")
        print(f"      Duty                       : {abs(k100_duty_kjph)/3600.0:>12,.2f}  kW")
